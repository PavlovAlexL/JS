...
module.exports = function (command) {
    ...
    if (input[0] === 'ADD') {
        var name = input[1];
        var phone = input[2].split(',');
        if(phoneBook[name] === undefined){  //if (!phoneBook.hasOwnProperty(name)) {}
            phoneBook[name] = phone;
        } else {
            phone.forEach(function (item) {
               if(phoneBook[name].indexOf(item) === -1) phoneBook[name].push(item);
            });
        }
    } else if (input[0] === 'REMOVE_PHONE') {
        var phone = input[1];
        for(var name in phoneBook){
            if (phoneBook[name].indexOf(phone) !== -1){
                phoneBook[name].splice(phoneBook[name].indexOf(phone), 1);
